...
int main() {
    ...
    // 4. Menghitung rata-rata terendah dan tertinggi dari dua nilai yang dimasukkan berturut
    if (n < 2) {
        // Tidak ada pasangan untuk dihitung rata-ratanya
        min_avg = DBL_MAX; // Tetapkan sebagai indikator tidak ada
        max_avg = DBL_MIN; // Tetapkan sebagai indikator tidak ada
    } else {
        for (int i = 0; i < n - 1; i++) { // Loop hingga n-2 untuk pasangan (i, i+1)
            ...
            if (current_avg > max_avg) { // Tambahkan logika untuk max_avg
                max_avg = current_avg;
            }
        }
    }
    ...
    if (n >= 2) { // Tampilkan rata-rata hanya jika ada pasangan yang bisa dihitung
        ...
        printf("Rata-rata tertinggi dari dua nilai berturut: %.2f\n", max_avg); // Output rata-rata tertinggi
    } else {
